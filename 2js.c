...
static int signal2scaling_encode(const char *msgname, unsigned id, signal_t *sig, FILE *o, bool header, const char *god, dbc2js_options_t *copts) 
{
	assert(msgname);
	assert(sig);
	assert(o);
	...
	if (sig->scaling != 1.0 || sig->offset != 0.0)
		type = "double";
	fprintf(o, "int encode_can_0x%03x_%s(can_obj_%s_t *o, %s in)", id, sig->name, god, type);
	if (header)
		return fputs(";\n", o);
	fputs(" {\n", o);
...
static int signal2scaling_decode(const char *msgname, unsigned id, signal_t *sig, FILE *o, bool header, const char *god, dbc2js_options_t *copts) 
{
	assert(msgname);
	assert(sig);
	assert(o);
	...
	if (sig->scaling != 1.0 || sig->offset != 0.0)
		type = "double";
	fprintf(o, "int decode_can_0x%03x_%s(const can_obj_%s_t *o, %s *out)", id, sig->name, god, type);
	if (header)
		return fputs(";\n", o);
	fputs(" {\n", o);
	if (copts->generate_asserts) {
...
static int signal2scaling(const char *msgname, unsigned id, signal_t *sig, FILE *o, bool decode, bool header, const char *god, dbc2js_options_t *copts)
{
	assert(copts);
	if (decode)
...
static int msg_pack(can_msg_t *msg, FILE *c, const char *name, bool motorola_used, bool intel_used, const char *god, dbc2js_options_t *copts)
{
	assert(msg);
	assert(c);
	assert(name);
...
static int msg_unpack(can_msg_t *msg, FILE *c, const char *name, bool motorola_used, bool intel_used, const char *god, dbc2js_options_t *copts)
{
	assert(msg);
	assert(c);
	assert(name);
...
static int msg_print(can_msg_t *msg, FILE *c, const char *name, const char *god, dbc2js_options_t *copts)
{
	assert(msg);
	assert(c);
	assert(name);
 ...
 * 
 */
static int msg_print_json(can_msg_t *msg, FILE *c, const char *name, const char *god, dbc2js_options_t *copts)
{
	assert(msg);
	assert(c);
	assert(name);
...
static int msg2h(can_msg_t *msg, FILE *h, dbc2js_options_t *copts, const char *god)
{
	assert(msg);
	assert(h);
	assert(copts);
...
static int switch_function(FILE *c, dbc_t *dbc, char *function, bool unpack, 
		bool prototype, const char *datatype, bool dlc, const char *god, const dbc2js_options_t *copts)
{
	assert(c);
	assert(dbc);
	assert(function);
...
static int switch_function_print(FILE *c, dbc_t *dbc, bool prototype, const char *god, dbc2js_options_t *copts)
{
	assert(c);
	assert(dbc);
	assert(god);
