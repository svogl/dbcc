 ...
 * be asserted to be non-NULL, DLCs within range (0-8), ID within ranges
 * (29-bit), and other properties.
 *
...
static unsigned fix_start_bit(bool motorola, unsigned start, unsigned siglen) {
  if (motorola) start = (8 * (7 - (start / 8))) + (start % 8) - (siglen - 1);
  return start;
}

static const char *determine_unsigned_type(unsigned length) {
  const char *type = "uint64_t";
  if (length <= 32) type = "uint32_t";
  if (length <= 16) type = "uint16_t";
  if (length <= 8) type = "uint8_t";
  return type;
}

static const char *determine_signed_type(unsigned length) {
  const char *type = "int64_t";
  if (length <= 32) type = "int32_t";
  if (length <= 16) type = "int16_t";
  if (length <= 8) type = "int8_t";
  return type;
}

static const char *determine_type(unsigned length, bool is_signed) {
  return is_signed ? determine_signed_type(length)
...
static int comment(signal_t *sig, FILE *o, const char *indent) {
  ...
  assert(o);
  return fprintf(o,
                 "%s/* %s: start-bit %u, length %u, endianess %s, scaling %g, "
                 "offset %g */\n",
                 indent, sig->name, sig->start_bit, sig->bit_length,
...
static int signal2deserializer(signal_t *sig, const char *msg_name, FILE *o,
  ...
  const unsigned start =
      fix_start_bit(motorola, sig->start_bit, sig->bit_length);
  const unsigned length = sig->bit_length;
  const uint64_t mask =
      length == 64 ? 0xFFFFFFFFFFFFFFFFuLL : (1uLL << length) - 1uLL;

  if (comment(sig, o, indent) < 0) return -1;
  ...
  if (sig->is_signed) {
    ...
    if (length <= 32) negative &= 0xFFFFFFFF;
    if (length <= 16) negative &= 0xFFFF;
    if (length <= 8) negative &= 0xFF;
    if (negative)
...
static int signal2serializer(signal_t *sig, const char *msg_name, FILE *o,
  ...
  uint64_t mask = sig->bit_length == 64 ? 0xFFFFFFFFFFFFFFFFuLL
  ...
  if (sig->is_floating) {
    ...
    fprintf(o, "%sx = ((%s)(o->%s.%s)) & 0x%" PRIx64 ";\n", indent,
            determine_unsigned_type(sig->bit_length), msg_name, sig->name,
            mask);
  }
  if (start) fprintf(o, "%sx <<= %u; \n", indent, start);
  fprintf(o, "%s%c |= x;\n", indent, motorola ? 'm' : 'i');
  return 0;
}

static int signal2print_json(signal_t *sig, unsigned id, const char *msg_name,
                             FILE *o, int add_comma) {
  char comma[] = ",";
  if (!add_comma) {
    comma[0] = ' ';
  }
  ...
  if (sig->is_floating)
    return fprintf(o,
                   "\tr = fprintf(data, \"%s = %%.3f (wire: %%g)\\n\", scaled, "
                   "(double)(print->%s));\n",
                   sig->name, sig->name);
  return fprintf(o,
                 "\tr = fprintf(data, \"%s = %%.3f (wire: %%.0f)\\n\", scaled, "
                 "(double)(print->%s));\n",
                 sig->name, sig->name);
}
...
static int signal2print_delta_json(signal_t *sig, unsigned id,
  ...
  if (sig->is_floating) {
    ...
    return fprintf(
        o,
        "\tr += snprintf(buf+r, bufSize-r,  \"\\\"%s\\\":%%g%s\", f);\n\t}\n",
        sig->name, comma);
  } else {
    if (sig->bit_length <= 8) {
      fprintf(o, "\t\tdecode_can_0x%3x_%s(o, &i);\n", id, sig->name, msg_name,
...
static int signal2print(signal_t *sig, unsigned id, const char *msg_name,
                        FILE *o) {
  if (sig->is_floating) {
    return fprintf(o, "\to['%s'] = \"%s.%s\" \n", sig->name, msg_name,
                   sig->name);
  }
  return fprintf(o, "\to['%s'] = %%.0f \t\t/* o->%s.%s */\n", sig->name,
                 msg_name, sig->name);

  /*super lazy*/
  if (sig->is_floating)
    return fprintf(o,
                   "\tr = print_helper(r, fprintf(output, \"%s = (wire: "
                   "%%g)\\n\", (double)(o->%s.%s)));\n",
...
static int signal2type(signal_t *sig, FILE *o) {
  ...
  if (sig->comment) {
    ...
    return fprintf(
        o, "\t/* scaling %.1f, offset %.1f, units %s %s */\n\t%s %s;\n",
        sig->scaling, sig->offset, sig->units[0] ? sig->units : "none",
        sig->is_floating ? ", floating" : "", type, sig->name);
  } else {
    return fprintf(o, "\t%s %s; /* scaling %.1f, offset %.1f, units %s %s */\n",
                   type, sig->name, sig->scaling, sig->offset,
                   sig->units[0] ? sig->units : "none",
                   sig->is_floating ? ", floating" : "");
  }
}

static int signal2type_json(signal_t *sig, FILE *o) {
  assert(sig);
  assert(o);
  const unsigned length = sig->bit_length;
  const char *type = determine_type(length, sig->is_signed);
  ...
  if (sig->comment) {
    ...
    return fprintf(o,
                   "\t/*%s*/ \t'%s' : { 'scaling': %.1f, 'offset': %.1f, "
                   "'units': '%s' %s}",
                   type, sig->name, sig->scaling, sig->offset,
                   sig->units[0] ? sig->units : "none",
                   sig->is_floating ? ", 'f':'floating'" : "");
  }
}
...
static uint64_t unsigned_max(signal_t *sig) {
  assert(sig);
  if (sig->bit_length == 64) return UINT64_MAX;
  return (1uLL << (sig->bit_length)) - 1uLL;
}

static int64_t signed_max(signal_t *sig) {
  assert(sig);
  if (sig->bit_length == 64) return INT64_MAX;
  return ((1uLL << (sig->bit_length - 1)) - 1uLL);
}

static int64_t signed_min(signal_t *sig) {
  assert(sig);
  if (sig->bit_length == 64) return INT64_MIN;
  return ~signed_max(sig);
}

static int signal2scaling_encode(const char *msgname, unsigned id,
                                 signal_t *sig, FILE *o, bool header,
  ...
  if (sig->scaling != 1.0 || sig->offset != 0.0) type = "double";
  fprintf(o, "int encode_can_0x%03x_%s(can_obj_%s_t *o, %s in)", id, sig->name,
          god, type);
  if (header) return fputs(";\n", o);
  fputs(" {\n", o);
  ...
  if (signal_are_min_max_valid(sig)) {
    ...
    if (gmin) fprintf(o, "\tif (in < %g)\n\t\treturn -1;\n", sig->minimum);
    if (gmax) fprintf(o, "\tif (in > %g)\n\t\treturn -1;\n", sig->maximum);
  }

  if (sig->scaling == 0.0) error("invalid scaling factor (fix your DBC file)");
  if (sig->offset != 0.0) fprintf(o, "\tin += %g;\n", -1.0 * sig->offset);
  if (sig->scaling != 1.0) fprintf(o, "\tin *= %g;\n", 1.0 / sig->scaling);
  fprintf(o, "\to->%s.%s = in;\n", msgname, sig->name);  // cast!
  return fputs("\treturn 0;\n}\n\n", o);
}

static int signal2scaling_decode(const char *msgname, unsigned id,
                                 signal_t *sig, FILE *o, bool header,
                                 const char *god, dbc2js_options_t *copts) {
  assert(msgname);
  assert(sig);
  ...
  const char *type = determine_type(sig->bit_length, sig->is_signed);
  if (sig->scaling != 1.0 || sig->offset != 0.0) type = "double";
  fprintf(o, "int decode_can_0x%03x_%s(const can_obj_%s_t *o, %s *out)", id,
          sig->name, god, type);
  if (header) return fputs(";\n", o);
  fputs(" {\n", o);
  if (copts->generate_asserts) {
  ...
  if (sig->scaling == 0.0) error("invalid scaling factor (fix your DBC file)");
  if (sig->scaling != 1.0) fprintf(o, "\trval *= %g;\n", sig->scaling);
  if (sig->offset != 0.0) fprintf(o, "\trval += %g;\n", sig->offset);
  if (signal_are_min_max_valid(sig)) {
    bool gmax = true;
    bool gmin = true;

    if (sig->is_signed) { /**@warning comparison may fail because of limits of
                             double size */
      gmin = sig->minimum > signed_min(sig);
      gmax = sig->maximum < signed_max(sig);
    } else {
...
static int signal2scaling(const char *msgname, unsigned id, signal_t *sig,
                          FILE *o, bool decode, bool header, const char *god,
...
static int print_function_name(FILE *out, const char *prefix, const char *name,
                               const char *postfix, bool in, char *datatype,
                               bool dlc, const char *god) {
  assert(out);
  assert(prefix);
  assert(name);
  assert(god);
  ...
  return fprintf(out, "static int %s_%s(can_obj_%s_t *o, %s %sdata%s)%s",
                 prefix, name, god, datatype, in ? "" : "*",
                 dlc ? ", uint8_t dlc, dbcc_time_stamp_t time_stamp" : "",
                 postfix);
}

static void make_name(char *newname, size_t maxlen, const char *name,
                      unsigned id) {
  assert(newname);
  assert(name);
...
static signal_t *find_multiplexor(can_msg_t *msg) {
  ...
  for (size_t i = 0; i < msg->signal_count; i++) {
    ...
    if (sig->is_multiplexor) {
      if (multiplexor)
        error(
            "multiple multiplexor values detected (only one per CAN msg is "
            "allowed) for %s",
            msg->name);
      multiplexor = sig;
    }
    if (sig->is_multiplexed) continue;
  }
  return multiplexor;
}

static signal_t *process_signals_and_find_multiplexer(can_msg_t *msg, FILE *c,
                                                      const char *name,
                                                      bool serialize) {
  assert(msg);
  assert(c);
  ...
  for (size_t i = 0; i < msg->signal_count; i++) {
    ...
    if (sig->is_multiplexed) continue;
    if ((serialize ? signal2serializer(sig, name, c, "\t")
                   : signal2deserializer(sig, name, c, "\t")) < 0)
...
static int multiplexor_switch(can_msg_t *msg, signal_t *multiplexor, FILE *c,
                              const char *msg_name, bool serialize) {
  assert(msg);
  assert(multiplexor);
  assert(c);
  ...
  for (size_t i = 0; i < msg->signal_count; i++) {
    ...
    fprintf(c, "\tcase %u:\n", sig->switchval);
    size_t j = i;
    for (; j < msg->signal_count &&
           msg->sigs[i]->switchval == msg->sigs[j]->switchval;
         j++) {
      assert(j < msg->signal_count);
      signal_t *sig = msg->sigs[j];
      if ((serialize ? signal2serializer(sig, msg_name, c, "\t\t")
                     : signal2deserializer(sig, msg_name, c, "\t\t")) < 0)
        return -1;
    }
    i = j - 1;
...
static int msg_pack(can_msg_t *msg, FILE *c, const char *name,
                    bool motorola_used, bool intel_used, const char *god,
                    dbc2js_options_t *copts) {
  assert(msg);
  assert(c);
  assert(name);
  ...
  if (message_has_signals) fprintf(c, "\tregister uint64_t x;\n");
  if (motorola_used) fprintf(c, "\tregister uint64_t m = 0;\n");
  if (intel_used) fprintf(c, "\tregister uint64_t i = 0;\n");
  if (!message_has_signals) fprintf(c, "\tUNUSED(o);\n\tUNUSED(data);\n");
  signal_t *multiplexor =
      process_signals_and_find_multiplexer(msg, c, name, true);

  if (multiplexor)
    if (multiplexor_switch(msg, multiplexor, c, name, true) < 0) return -1;

  if (message_has_signals) {
    fprintf(c, "\t*data = %s%s%s%s%s;\n",
            swap_motorola && motorola_used ? "reverse_byte_order" : "",
            motorola_used ? "(m)" : "", motorola_used && intel_used ? "|" : "",
...
static int msg_unpack(can_msg_t *msg, FILE *c, const char *name,
                      bool motorola_used, bool intel_used, const char *god,
                      dbc2js_options_t *copts) {
  assert(msg);
  assert(c);
  ...
  signal_t *multiplexor =
      process_signals_and_find_multiplexer(msg, c, name, false);
  if (multiplexor)
    if (multiplexor_switch(msg, multiplexor, c, name, false) < 0) return -1;
  fprintf(c, "\to->%s_rx = 1;\n", name);
...
static int msg_print(can_msg_t *msg, FILE *c, const char *name, const char *god,
                     dbc2js_options_t *copts) {
  assert(msg);
  assert(c);
  assert(name);
  ...
  if (msg->signal_count)
    ...
    fprintf(c, "\tUNUSED(o);\n\tUNUSED(output);\n");
  for (size_t i = 0; i < msg->signal_count; i++) {
    if (signal2print(msg->sigs[i], msg->id, name, c) < 0) return -1;
  }
  if (msg->signal_count)
    fprintf(c, "\treturn r;\n}\n\n");
  else
 ...
 *
 */
static int msg_print_json(can_msg_t *msg, FILE *c, const char *name,
                          const char *god, dbc2js_options_t *copts) {
  assert(msg);
  assert(c);
  assert(name);
  ...
  for (size_t i = 0; i < msg->signal_count; i++) {
    if (signal2print_json(msg->sigs[i], msg->id, name, c,
...
static int msg_print_delta_json(can_msg_t *msg, FILE *c, const char *name,
  ...
  assert(copts);
  fprintf(c,
          "int print_delta_%s(const can_obj_%s_t *o, const can_obj_%s_t *old, "
 ...
 *
 */
static int msg_print_cond_json(can_msg_t *msg, FILE *c, const char *name,
                               const char *god, dbc2js_options_t *copts) {
  assert(msg);
  assert(c);
  assert(name);
...
static int msg_dlc_check(can_msg_t *msg) {
  ...
  if (used > bits) {
    warning(
        "Too many signals, not enough bytes (DLC is too low, fix your DBC "
        "file): %s",
        msg->name);
    return -1;
  }
...
static int msg2c(can_msg_t *msg, FILE *c, dbc2js_options_t *copts, char *god) {
  assert(msg);
  assert(c);
  assert(copts);
  assert(god);
  ...
  if (copts->generate_pack &&
      msg_pack(msg, c, name, motorola_used, intel_used, god, copts) < 0)
    return -1;

  if (copts->generate_unpack &&
      msg_unpack(msg, c, name, motorola_used, intel_used, god, copts) < 0)
    return -1;

  for (size_t i = 0; i < msg->signal_count; i++) {
    if (copts->generate_unpack)
      if (signal2scaling(name, msg->id, msg->sigs[i], c, true, false, god,
                         copts) < 0)
        return -1;
    if (copts->generate_pack)
      if (signal2scaling(name, msg->id, msg->sigs[i], c, false, false, god,
                         copts) < 0)
        return -1;
  }
  ...
  if (copts->generate_print &&
      msg_print_delta_json(msg, c, name, god, copts) < 0)
    return -1;

  if (copts->generate_print &&
      msg_print_cond_json(msg, c, name, god, copts) < 0)
    return -1;

  return 0;
}

static int msg2h(can_msg_t *msg, FILE *h, dbc2js_options_t *copts,
                 const char *god) {
  assert(msg);
  assert(h);
  assert(copts);
  ...
  for (size_t i = 0; i < msg->signal_count; i++) {
    if (copts->generate_unpack)
      if (signal2scaling(name, msg->id, msg->sigs[i], h, true, true, god,
                         copts) < 0)
        return -1;
    if (copts->generate_pack)
      if (signal2scaling(name, msg->id, msg->sigs[i], h, false, true, god,
                         copts) < 0)
        return -1;
  }
  fputs("\n\n", h);
...
static const char *cfunctions_print_only =
    ...
    "\treturn ((r >= 0) && (print_return_value >= 0)) ? r + print_return_value "
    ": -1;\n"
    "}\n\n";
...
static int switch_function(FILE *c, dbc_t *dbc, char *function, bool unpack,
                           bool prototype, const char *datatype, bool dlc,
                           const char *god, const dbc2js_options_t *copts) {
  assert(c);
  assert(dbc);
  assert(function);
  assert(god);
  assert(copts);
  fprintf(
      c, "int %s_message(can_obj_%s_t *o, const unsigned long id, %s %sdata%s)",
  ...
  if (copts->generate_asserts) {
    ...
    if (dlc)
      fprintf(c,
              "\tassert(dlc <= 8);         /* Maximum of 8 bytes in a CAN "
              "packet */\n");
  }
  ...
  for (size_t i = 0; i < dbc->message_count; i++) {
    ...
    fprintf(c, "\tcase 0x%03lx: return %s_%s(o, data%s);\n", msg->id, function,
            name, dlc ? ", dlc, time_stamp" : "");
  }
  fprintf(c, "\tdefault: break; \n\t}\n");
  return fprintf(c, "\treturn -1; \n}\n\n");
}

static int switch_function_print(FILE *c, dbc_t *dbc, bool prototype,
                                 const char *god, dbc2js_options_t *copts) {
  assert(c);
  assert(dbc);
  assert(god);
  assert(copts);
  fprintf(c,
          "int print_message(const can_obj_%s_t *o, const unsigned long id, "
          "char* buf, int bufSize )",
          god);
  if (prototype) return fprintf(c, ";\n");
  fprintf(c, " {\n");
...
static int switch_function_conditional_print(FILE *c, dbc_t *dbc,
                                             bool prototype, const char *god,
                                             dbc2js_options_t *copts) {
  assert(c);
  assert(dbc);
  assert(god);
  assert(copts);
  fprintf(c,
          "int print_message_conditional(const can_obj_%s_t *o, const unsigned "
          "long id, char* buf, int bufSize )",
          god);
  if (prototype) return fprintf(c, ";\n");
  fprintf(c, " {\n");
  if (copts->generate_asserts) {
...
static int msg2h_types_json(dbc_t *dbc, FILE *h) {
  ...
  for (size_t i = 0; i < dbc->message_count; i++) {
    ...
    if (msg->comment) fprintf(h, "/* %s */\n", msg->comment);

    fprintf(h, "let META_%s = {\n", name);
    for (size_t i = 0; i < msg->signal_count; i++) {
      if (signal2type(msg->sigs[i], h) < 0) return -1;
      if (i < msg->signal_count - 1) {
        fprintf(h, ",\n");
      } else {
        fprintf(h, "\n");
      }
...
static char *msg2h_god_object(dbc_t *dbc, FILE *h, const char *name) {
  ...
  fprintf(h, "typedef PREPACK struct {\n");
  for (size_t i = 0; i < dbc->message_count; i++)
    if (msg_data_type_time_stamp(h, dbc->messages[i]) < 0) goto fail;
  for (size_t i = 0; i < dbc->message_count; i++)
    if (msg_data_type_bitfields(h, dbc->messages[i]) < 0) goto fail;
  for (size_t i = 0; i < dbc->message_count; i++)
    if (msg_data_type(h, dbc->messages[i], false) < 0) goto fail;
...
/********** GENERATE META DATA
 */
static void make_meta_name(char *newname, size_t maxlen, const char *name,
                           unsigned id) {
  assert(newname);
  assert(name);
...
static int meta_signal2Vtype(char *thetype, size_t maxlen, signal_t *sig) {
  assert(sig);
  assert(thetype);
  const unsigned length = sig->bit_length;
  const char *type = determine_type(length, sig->is_signed);
  ...
  if (sig->is_floating) {
    ...
    if (sig->bit_length == 1) {
      ...
      type = "u64";
      if (length <= 32) type = "u32";
      if (length <= 16) type = "u16";
      if (length <= 8) type = "u8";
    } else {
      type = "i64";
      if (length <= 32) type = "i32";
      if (length <= 16) type = "i16";
      if (length <= 8) type = "i8";
    }
  }
  return snprintf(thetype, maxlen, "%s", type);
...
static int switch_function_print_meta_header(FILE *h, dbc_t *dbc,
                                             bool prototype, const char *god,
                                             dbc2js_options_t *copts) {
  assert(h);
  assert(dbc);
  ...
  fprintf(h, "struct meta_signal_info;   /* forward decl */\n\n");
  fprintf(h,
  ...
  fprintf(h,
          ...
          "} value_type;\n"
          "\n"
          "typedef void (*filter_func)(struct meta_channel_info* chan, struct "
          "meta_signal_info* sig, struct _filter_data* filt, value_type val);\n"
          "\n"
          "struct _filter_data {\n"
          ...
          "\n"
          "struct meta_signal_info {\n"
          "   int id;\n"
          "   char name[%d];\n"
          "   char units[%d];\n"
          "   char type[%d];\n"
          "   value_type_def vtype;\n"
          "   value_type current;\n"
          "   value_type old;\n"
          "   double min;\n"
          "   double max;\n"
  ...
  fprintf(h,
          "// per-message type meta-data list; indexed by the position in the "
          "channel list\n");
  fprintf(h, "extern struct meta_signal_info* meta_signal_infos[];\n\n");
  fprintf(
      h,
  ...
  fprintf(h,
          "extern struct meta_signal_info* find_signal_info(int "
          "channelid, const char* signalname);\n\n");
  fprintf(h,
          "// enable/disable a signal for printing; pass the channel id and "
          "signal name (string match)\n");
  fprintf(h, "// returns -1 if not found, 0 on success\n");
  fprintf(h,
          "extern int enable_signal(int channelid, const char* signalname);\n");
  fprintf(
      h, "extern int disable_signal(int channelid, const char* signalname);\n");
...
static char *meta_funcs =
    "\n"
    "static inline size_t find_channel_index(int channelid) {\n"
    "\n"
    ...
    "}\n"
    "\n"
    "struct meta_signal_info* find_signal_info(int channelid, const char* "
    "signalname)\n"
    "{\n"
    "  int idx = find_channel_index(channelid);\n"
    "  if (idx<0) {\n"
...
static int print_message_meta(FILE *c, dbc_t *dbc, dbc2js_options_t *copts,
                              can_msg_t *msg) {
  assert(c);
  assert(dbc);
  assert(msg);
  ...
  fprintf(c, "struct meta_signal_info %s[] = {\n", msg_name);
  for (size_t i = 0; i < msg->signal_count; i++) {
    signal_t *sig = msg->sigs[i];
    char thetype[MAX_NAME_LENGTH];
    char theVtype[MAX_NAME_LENGTH];

    meta_signal2type(thetype, MAX_NAME_LENGTH, sig);
    meta_signal2Vtype(theVtype, MAX_NAME_LENGTH, sig);
    ...
    //		if (sig->minimum)
    fprintf(c, "    , .min=%g\n", sig->minimum);
    if (sig->maximum) fprintf(c, "    , .max=%g\n", sig->maximum);
    if (sig->comment) {
      fprintf(c, "    , .comment=\"%s\"\n", sig->comment);
    }
    fprintf(c, "    , .filter=NULL");
    fprintf(c, "  },\n", i);
  }
  fprintf(c, "  { .id = -1 }\n");
  fprintf(c, "};\n\n", msg_name);
  ...
  static int switch_function_print_meta(FILE * c, dbc_t * dbc, bool prototype,
                                        const char *god,
                                        dbc2js_options_t *copts) {
    assert(c);
    assert(dbc);
    ...
    for (size_t i = 0; i < dbc->message_count; i++) {
      ...
      if (msg->comment) {
        fprintf(c,
                "\t{ /* %4d */  .id=0x%03lx, .name=\"%s\", .enabled=0 },  /* "
                "%s */\n",
    ...
    fprintf(c, "struct meta_signal_info* meta_signal_infos[] = {\n");
    for (size_t i = 0; i < dbc->message_count; i++) {
      can_msg_t *msg = dbc->messages[i];
      char name[MAX_NAME_LENGTH] = {0};
  ...
  int dbc2js(dbc_t * dbc, FILE * c, FILE * h, const char *name,
             dbc2js_options_t *copts) {
    ...
    struct tm *timeinfo =
        localtime(&rawtime);  // This is not considered safe on Visual Studio
    char *god = NULL;
     ...
     * alpha only*/
    if (!isalpha(file_guard[0])) file_guard[0] = '_';
    for (size_t i = 0; i < file_guard_len; i++)
      file_guard[i] = (isalnum(file_guard[i])) ? toupper(file_guard[i]) : '_';

    /* sort signals by id */
    qsort(dbc->messages, dbc->message_count, sizeof(dbc->messages[0]),
          message_compare_function);

    /* sort by size for better struct packing */
    for (size_t i = 0; i < dbc->message_count; i++) {
      can_msg_t *msg = dbc->messages[i];
      qsort(msg->sigs, msg->signal_count, sizeof(msg->sigs[0]),
            signal_compare_function);
    }

    /* header file (begin) */
    fprintf(h,
            "/** CAN message encoder/decoder: automatically generated - do not "
            "edit\n");
    ...
    fprintf(
        h,
        "typedef uint32_t dbcc_time_stamp_t; /* Time stamp for message; you "
        "decide on units */\n");
    fprintf(h, "#endif\n\n");
    ...
    fprintf(
        h,
        "\tDBCC_SIG_STAT_ERROR_E         = 2, /* Encode/Decode/Timestamp/Any "
        "error */\n");
    ...
    if (copts->generate_unpack)
      switch_function(h, dbc, "unpack", true, true, "uint64_t", true, god,
                      copts);

    if (copts->generate_pack)
      switch_function(h, dbc, "pack", false, true, "uint64_t", false, god,
                      copts);
    ...
    for (size_t i = 0; i < dbc->message_count; i++)
      if (msg2h(dbc->messages[i], h, copts, god) < 0) return -1;
    ...
    fputs(
        "/*    ... but forked by svogl, see "
        "<https://github.com/voxel-dot-at/dbcc> */\n",
        c);
    fprintf(c, "#include \"%s\"\n", name);
    ...
    if (dbc->use_float)
      fprintf(
          c,
          "#include <math.h> /* uses macros NAN, INFINITY, signbit, no need "
          "for -lm */\n");
    if (copts->generate_asserts) fprintf(c, "#include <assert.h>\n");
    fputc('\n', c);
    fprintf(c, "#define UNUSED(X) ((void)(X))\n\n");
    fprintf(c, "#include <string.h>\n");  // for the meta-ops
    fputs(cfunctions, c);
    if (copts->generate_print) fputs(cfunctions_print_only, c);

    if (copts->generate_unpack && dbc->use_float) fputs(float_unpack, c);
    if (copts->generate_pack && dbc->use_float) fputs(float_pack, c);
    ...
    if (copts->generate_unpack)
      switch_function(c, dbc, "unpack", true, false, "uint64_t", true, god,
                      copts);

    if (copts->generate_pack)
      switch_function(c, dbc, "pack", false, false, "uint64_t", false, god,
                      copts);
